...
const extendedMetadataRequest = protobuf.Root.fromJSON(
  extendedMetadataJsonDescriptor,
).lookup("Message");
const audioFeaturesResponse = protobuf.Root.fromJSON(
  audioFeaturesJsonDescriptor,
).lookup("Message");
const trackMetadataResponse = protobuf.Root.fromJSON(
...
`,
      ...
      try {
        ...
        if (featuresRes) {
          ...
          featuresRes.forEach((track) => {
            if (track) {
              const trackDetails = metadataRes?.find(
                (t) => t?.id === track?.id,
              );
              if (trackDetails) {
    ...
    if (info) {
      ...
      if (CONFIG.isRichUiEnabled) {
        ...
        if (CONFIG.isEnergyEnabled)
          bottomStats.push(`E: ${Math.round(parsedInfo.energy * 100)}`);
        ...
        if (CONFIG.isYearEnabled)
          bottomStats.push(`${parsedInfo.release_date}`);
  ...
  const reRenderAll = () => {
    ...
    if (recommendations) {
      const tracks = recommendations.getElementsByClassName(
        "main-trackList-trackListRow",
      );
      for (const track of tracks) {
  ...
  const updateTracklist = (tracklist) => {
    ...
    if (tracklistHeader && !tracklistHeader.querySelector(".djinfoheader")) {
      ...
      const updateHeaderGrid = () => {
        let djInfoH =
          tracklistHeader.querySelector(".djinfoheader")?.parentElement;
  ...
  const updateRecommendations = (recommendations) => {
    ...
    const tracklist = recommendations.querySelector(
